 ...
 * @name rolnikMock
 * @description
 * # rolnikMock for mocking http requests
 * # needs to be included to Rolnik module
 */
angular.module('rolnikMock')
.run(mockRun);
...
function mockRun($httpBackend) {
	...
	// 		return [200, mock.selectedProducts];
	// 	});

	$httpBackend.whenPUT(/api\/users\/([0-9]*)/)
		.respond(function (type, path, data) {
			console.log(type, path, data);
			return [200, data];
		});

	$httpBackend.whenDELETE(/api\/users\/([0-9]*)/)
		.respond(function (type, path, data) {
			console.log(type, path, data);
			return [200, mock.usersList[0]];
		});

		//
	$httpBackend.whenGET(/api\/products\/([0-9]*)/)
		.respond(function (type, path, data) {
			console.log(type, path, data);
			var searchKey = "";
			_( _.drop(path, 13)).forEach(function(n) {
			  searchKey += n
			}).value();
			return [200, findByName(searchKey)];
		});
	$httpBackend.whenGET('api/products')
		.respond(function (type, path, data) {
			console.log(type, path, data);
...
mock.selectedProducts = [
    ...
    function findByName(searchKey)  {
        var results = mock.selectedProducts.filter(function(element) {
